...
def mayores(lista : List[(String, Int)])  = {
    val x = lista.filter(._2 > promedio(lista)).map(._1)
}
...
lista.groupBy{case(_, goles) => goles}.map(s => s.match{case (goles, lista) => (goles, lista.size)}).toList.sortBy(_._2).reverse